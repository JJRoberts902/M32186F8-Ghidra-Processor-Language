...
FPREL:		rel					is	rel16	[ rel = rel16 + $(FP_VALUE); ] { export *:4 rel; }
FPREL_1:	rel					is	rel16	[ rel = rel16 + $(FP_VALUE); ] { export *:1 rel; }
FPREL_2:	rel					is	rel16	[ rel = rel16 + $(FP_VALUE); ] { export *:2 rel; }
FP_ADDRESS:	rel					is	rel16	[ rel = rel16 + $(FP_VALUE); ] { export &:4 rel; }
...
:BSET	"#"bitpos, "@("rel16,Rsrc")" 	is op1=10 & op2_1=0 & bitpos; op3=6 & Rsrc; rel16 {
	local address = (Rsrc + rel16);
	*:1 address = (*:1 address | (1 << (7 - bitpos))):1;
}
...
:CLRPSW	"#"imm8					is op12=0x72; imm8 {
	PSW = PSW & (0xffffffff ^ zext(imm8:1));
}
...
:SUBX	Rdest, Rsrc		is op1=0 & Rdest; op3=1 & Rsrc { #fix
	# Conservative SUBX model for decompiler flow: subtract source and update borrow flag.
	local olddest = Rdest;
	Rdest = Rdest - Rsrc - zext($(C));
	$(C) = sborrow(olddest, Rsrc);
	CBR = zext(sborrow(olddest, Rsrc));
}
